...
float ECC33pathLoss(float f, float TxH, float RxH, float d, int mode)
{
...
*/
	...
	float Gr = 0.759 * RxH - 1.862;	// Big city with tall buildings (1)
	// PL = Afs + Abm - Gb - Gr
	float Afs = 92.4 + 20 * log10(d) + 20 * log10(f);
	float Abm =
	    20.41 + 9.83 * log10(d) + 7.894 * log10(f) +
	    9.56 * (log10(f) * log10(f));
	float Gb = log10(TxH / 200) * (13.958 + 5.8 * (log10(d) * log10(d)));
