...
/*=============================================================================
 ...
 |
 *===========================================================================*/
float FFunction(const float td)
{
    // constants from [Algorithm, 6.9]
    const float a[] = { 133.4, 104.6, 71.8 };
    const float b[] = { 0.332e-3, 0.212e-3, 0.157e-3 };
    const float c[] = { -10, -2.5, 5 };
    ...
    const float F_0 = a[i] + b[i] * td + c[i] * log10(td);  // [Algorithm, 6.9]
...
/*=============================================================================
 ...
 |
 *===========================================================================*/
float TroposcatterLoss(const float d__meter, const float theta_hzn[2], const float d_hzn__meter[2], const float h_e__meter[2], 
    const float a_e__meter, const float N_s, const float f__mhz, const float theta_los, float *h0)
{
    ...
    {
        float ad = d_hzn__meter[0] - d_hzn__meter[1];
        float rr = h_e__meter[1] / h_e__meter[0];
        ...
        const float theta = theta_hzn[0] + theta_hzn[1] + d__meter / a_e__meter;  // angular distance, in radians

        // [TN101, Eqn 9.4a]
        const float r_1 = 2.0 * wn * theta * h_e__meter[0];
        const float r_2 = 2.0 * wn * theta * h_e__meter[1];

        if (r_1 < 0.2 && r_2 < 0.2)
            return 1001;                // "If both r_1 and r_2 are less than 0.2 the function A_scat is not defined (or is infinite)" [Algorithm, page 11]

        float s = (d__meter - ad) / (d__meter + ad);       // asymmetry parameter

        // "In all of this, we truncate the values of s and q at 0.1 and 10" [Algorithm, page 16]
        const float q = MIN(MAX(0.1, rr / s), 10.0);       // TN101, Eqn 9.5
        s = MAX(0.1, s);                                    // TN101, Eqn 9.5

        float h_0__meter = (d__meter - ad) * (d__meter + ad) * theta * 0.25 / d__meter;   // height of cross-over, [Algorithm, 4.66] [TN101v1, 9.3b]

        constexpr float Z_0__meter = 1.7556e3;             // Scale height, [Algorithm, 4.67]
        constexpr float Z_1__meter = 8.0e3;                // [Algorithm, 4.67]
        const float eta_s = (h_0__meter / Z_0__meter) * (1.0 + (0.031 - N_s * 2.32e-3 + pow(N_s, 2) * 5.67e-6) * exp(-pow(MIN(1.7, h_0__meter / Z_1__meter), 6)));  // Scattering efficiency factor, eta_s [TN101 Eqn 9.3a]

        const float H_00 = (H0Function(r_1, eta_s) + H0Function(r_2, eta_s)) / 2;                        // First term in TN101v1, Eqn 9.5
        const float Delta_H_0 = MIN(H_00, 6.0 * (0.6 - log10(MAX(eta_s, 1.0))) * log10(s) * log10(q));
    ...
    *h0 = H_0;
    const float th = d__meter / a_e__meter - theta_los;

    constexpr float D_0__meter = 40e3;   // [Algorithm, 6.8]
    constexpr float H__meter = 47.7;     // [Algorithm, 4.63]
